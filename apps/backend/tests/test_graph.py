...
async def test_parallel_tool_calls(
    ...
    """
    with patch(f"{graph_import_path}.create_react_agent") as mock_create_react_agent:
...
def test_get_model(graph_module, chat_model, openai_base_url):
    """Test whether our chat model correctly sets and defaults to gpt-4o-mini."""

    model = graph_module.get_model()

    assert isinstance(model, ChatOpenAI)

    if chat_model:
        assert model.model_name == chat_model
    else:
