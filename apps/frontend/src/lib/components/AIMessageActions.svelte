...
<div class="absolute bottom-2 left-0 flex items-center gap-1 transition-all duration-300 ease-in-out"
    ...
    <FlowbiteClipboard value={message.text} embedded color="alternative" class="p-1.5!">
        {#snippet children(success)}
            <Tooltip placement="top" isOpen={success}> {success ? m.message_copied() : m.message_copy()}
            </Tooltip>
            {#if success}
                <Check class="w-3 h-3" />
            {:else}
