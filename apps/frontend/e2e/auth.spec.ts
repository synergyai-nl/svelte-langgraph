...
test.describe('When unauthenticated', () => {
	...
	test.describe('On "/chat/"', () => {
		...
		test('should not show greeting', async ({ chat }) => {
			await expect(chat.greeting).not.toBeVisible();
		});
...
test.describe('When authenticated', () => {
	...
	test.describe('On "/chat/"', () => {
		...
		test('should show chat interface', async ({ chat }) => {
			await expect(chat.heading).toBeVisible({ timeout: 15000 });
		});

		test('should show greeting', async ({ chat }) => {
			await expect(chat.greeting).toBeVisible();
		});
